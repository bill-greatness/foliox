...
export const Details = ({ data, close }) => {
  return (
    <motion.div
      ...
      animate={{y:0, opacity:1, originY:20}}
      exit={{y:100, opacity:0, originY:0}}
      transition={{ delay:0.2, duration: 1.2}}
    className="flex items-center justify-center">
      <div className="flex flex-col fixed bottom-0 h-[80vh] rounded-t-lg overflow-y-auto w-[98vw] md:w-[90%] items-center bg-gray-400">
        ...
        <div className="mt-10 md:p-5  w-full bg-white">
          <div className="flex w-full flex-col md:justify-start md:flex-row gap-2">
            <div style={{backgroundImage:`url(${data.cover})`}} className="bg-cover bg-center bg-no-repeat w-full h-48 md:w-40 md:h-40 shrink-0"></div>
          ...
          {/* Links & Credentials */}
          <div className="flex flex-col p-2">
            <Link href={data.link} target="_blank" rel="noopener" className="flex items-center text-md my-4 gap-1">
              {" "}
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="flex items-center"><TbIdBadge className="text-xl"/>: {`{request_for_id}`}</p>
              <p className="flex items-center"><TbLockSquareRounded className="text-xl"/>: {`{request_for_password}`}</p>
